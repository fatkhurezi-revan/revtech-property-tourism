      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 lg:gap-16 mb-16">
          ...
          <div className="space-y-4">
            <Link href="/" className="flex items-center gap-3 inline-flex group" aria-label={`Beranda ${siteConfig.name}`}>
              ...
              <div className="flex flex-col">
                <span className="text-3xl font-serif font-bold text-white leading-none">{siteConfig.name}</span>
                <span className="text-xs uppercase tracking-widest text-primary-foreground/70 font-medium leading-tight mt-1.5">{siteConfig.subtitle}</span>
              </div>
            </Link>
          ...
          <nav aria-label="Tautan Cepat Footer">
            ...
            <ul className="space-y-4">
              ...
              <li><Link href="/about" className="text-sm text-primary-foreground/80 hover:text-white transition-colors duration-300 ease-in-out">Tentang Kami & Layanan</Link></li>
          ...
          <address className="not-italic">
            ...
            <a href={`https://wa.me/${siteConfig.contact.whatsappNumber}?text=${encodeURIComponent(siteConfig.contact.whatsappMessage)}`} target="_blank" rel="noopener noreferrer" aria-label="Chat via WhatsApp" className="inline-block mt-8 bg-secondary text-secondary-foreground px-6 py-3 rounded-full text-sm font-medium hover:bg-secondary-dark transition-all duration-300 ease-in-out shadow-md hover:shadow-lg hover:-translate-y-1">
              Chat via WhatsApp
            </a>
          </address>
        </div>
