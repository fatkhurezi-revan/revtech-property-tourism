    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          <Link href="/" className="flex items-center gap-3 group" aria-label={`Beranda ${siteConfig.name}`}>
            ...
            <div className="flex flex-col">
              <span className={`text-2xl font-serif font-bold leading-none transition-colors duration-300 ${isScrolled ? 'text-primary' : 'text-white'}`}>{siteConfig.name}</span>
              <span className={`text-[0.65rem] uppercase tracking-widest font-medium leading-tight mt-1 transition-colors duration-300 ${isScrolled ? 'text-muted-foreground' : 'text-white/80'}`}>{siteConfig.subtitle}</span>
            </div>
          </Link>
          ...
          <nav className="hidden md:flex space-x-8" aria-label="Navigasi Utama">
            ...
            <Link href="/about" className={`text-sm font-medium transition-all duration-300 ease-in-out hover:-translate-y-0.5 ${isScrolled ? 'text-foreground hover:text-primary' : 'text-white/90 hover:text-white'}`}>Tentang & Layanan</Link>
      ...
      {/* Mobile Menu */}
      <div
        className={`md:hidden bg-background border-b border-border overflow-hidden text-foreground shadow-lg transition-all duration-300 ease-in-out ${isMobileMenuOpen ? "max-h-96 opacity-100" : "max-h-0 opacity-0"}`}
      >
        <nav className="px-4 pt-2 pb-6 space-y-4 flex flex-col" aria-label="Navigasi Seluler">
          ...
          <Link href="/about" className="text-base font-medium hover:text-primary transition-colors duration-300 ease-in-out" onClick={() => setIsMobileMenuOpen(false)}>Tentang & Layanan</Link>
          ...
          <a href={`https://wa.me/${siteConfig.contact.whatsappNumber}?text=${encodeURIComponent(siteConfig.contact.whatsappMessage)}`} target="_blank" rel="noopener noreferrer" className="bg-primary text-primary-foreground px-6 py-3 rounded-full text-center text-sm font-medium hover:bg-primary-dark transition-all duration-300 ease-in-out shadow-md hover:shadow-lg inline-block mt-4">
            Chat via WhatsApp
          </a>
        </nav>
      </div>
